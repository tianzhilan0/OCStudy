...
- (SDWebImageCombinedOperation *)loadImageWithURL:(nullable NSURL *)url
    ...
    // 很常见的错误是使用NSString对象而不是NSURL来发送URL。出于某种奇怪的原因，Xcode不会这样做
    // 对于这种类型不匹配，抛出任何警告。这里我们通过允许url作为NSString传递来防止这个错误。
    if ([url isKindOfClass:NSString.class]) {
        url = [NSURL URLWithString:(NSString *)url];
    }

    // 防止应用崩溃的参数类型错误，如发送NSNull而不是NSURL
...
- (void)callDownloadProcessForOperation:(nonnull SDWebImageCombinedOperation *)operation
    ...
    if (shouldDownload) {
        if (cachedImage && options & SDWebImageRefreshCached) {
            // 如果在缓存中找到了映像，但是提供了SDWebImageRefreshCached，则通知缓存的映像
            // 并尝试重新下载，以便有机会让NSURLCache刷新它从服务器
            [self callCompletionBlockForOperation:operation completion:completedBlock image:cachedImage data:cachedData error:nil cacheType:cacheType finished:YES url:url];
            // 将缓存的映像传递给映像加载器。映像加载程序应该检查远程映像是否等于缓存的映像。
        ...
        operation.loaderOperation = [imageLoader requestImageWithURL:url options:options context:context progress:progressBlock completed:^(UIImage *downloadedImage, NSData *downloadedData, NSError *error, BOOL finished) {
            ...
            if (!operation || operation.isCancelled) {
                ...
                // 图像刷新命中NSURLCache缓存，不调用补全块
            } else if ([error.domain isEqualToString:SDWebImageErrorDomain] && error.code == SDWebImageErrorCancelled) {
                // 下载操作被用户取消发送请求前，不要阻止失败的URL
...
- (void)callStoreCacheProcessForOperation:(nonnull SDWebImageCombinedOperation *)operation
    ...
    if (shouldCacheOriginal) {
        // 通常使用存储缓存类型，但如果转换了目标图像，则使用原始存储缓存类型
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            @autoreleasepool {
                ...
                if (transformedImage && finished) {
                    ...
                    NSData *cacheData;
                    // 如果图像被转换，则传递nil，这样我们就可以重新计算图像中的数据
